...
import { formatDateEx, getAddressLine } from "@/lib/utils";
import { Basics } from "@/lib/json/reactive";
...
export function BasicDetails({ cvBasics, initials }: BasicDetailsProps) {
    return (
                ...
                <div className="lg:col-span-1">
                    <Card className="border-slate-200">
                        <CardContent className="p-6 text-center">
                            ...
                            <h2 className="text-2xl font-bold text-slate-900 mb-2">{cvBasics.name}</h2>
                            <p className="text-lg text-emerald-600 font-medium mb-4">{cvBasics.label}</p>
                    ...
                    <Card className="border-slate-200">
                        ...
                        <CardContent className="space-y-4">
                            <div className="flex items-center gap-3">
                                ...
                                <span className="text-slate-700">{cvBasics.email}</span>
                            </div>
                            <div className="flex items-center gap-3">
                                <Phone className="w-4 h-4 text-slate-500" />
                                <span className="text-slate-700">{cvBasics.phone}</span>
                            </div>
                            <div className="flex items-center gap-3">
                                <MapPin className="w-4 h-4 text-slate-500" />
                                <span className="text-slate-700">{getAddressLine(cvBasics.location)}</span>
                            </div>
                            {/* TODO: Only include website if provided. */}
                            <div className="flex items-center gap-3">
                                <Globe className="w-4 h-4 text-slate-500" />
                                <span className="text-slate-700"><a href={cvBasics.website}>{cvBasics.website}</a></span>
                            </div>
                        </CardContent>
                    </Card>
                        ...
                        <CardContent className="space-y-4">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <p className="text-slate-700">{formatDateEx(cvBasics.dateOfBirth, true)}</p>
                                </div>
                                <div>
                                    <label className="text-sm font-medium text-slate-500">Nationality</label>
                                    <p className="text-slate-700">{cvBasics.nationality}</p>
                                </div>
                                <div>
                                    <label className="text-sm font-medium text-slate-500">Work Authorization</label>
                                    <p className="text-slate-700">{cvBasics.workAuth}</p>
                                </div>
                                <div>
                                    <label className="text-sm font-medium text-slate-500">Availability</label>
                                    <p className="text-slate-700">{cvBasics.availability}</p>
                                </div>
                            </div>
                        </CardContent>
                    </Card>
